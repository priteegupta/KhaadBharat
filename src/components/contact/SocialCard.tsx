...
export const SocialCard: React.FC = () => {
  ...
  return (
    <div className="bg-white border border-brand-green/10 rounded-3xl p-6 md:p-8 mb-12 shadow-sm">
      {/* Header */}
      <div className="mb-6">
        <span className="inline-block px-3 py-1.5 text-[10px] md:text-xs font-black tracking-widest text-brand-green uppercase bg-brand-green/10 rounded-full mb-3 border border-brand-green/15 shadow-sm animate-pulse">
          {t("social.badge")}
        </span>
        ...
        {platforms.map((p) => {
          ...
          return (
            <motion.a
              ...
              className={`p-6 rounded-2xl border border-brand-green/10 shadow-sm flex flex-col items-center justify-center text-center transition-all duration-350 ${p.colorClass} group`}
              whileHover={{ scale: 1.03, y: -4 }}
            >
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-3 bg-brand-green-light group-hover:bg-white/20 transition-all duration-350 shadow-inner group-hover:rotate-6">
