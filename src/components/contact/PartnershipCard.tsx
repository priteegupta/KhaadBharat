  ...
  return (
    <div className="bg-brand-beige-cream/30 border border-brand-green/10 rounded-3xl p-8 md:p-12 mb-12 relative overflow-hidden">
      ...
      <div className="max-w-2xl mb-10">
        <span className="inline-block px-3 py-1 text-[10px] md:text-xs font-black tracking-widest text-brand-brown uppercase bg-brand-brown/10 rounded-full mb-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {partnerships.map(({ key, icon: Icon }) => (
          <motion.div
            key={key}
            className="p-6 rounded-2xl bg-white border border-brand-green/5 shadow-sm hover:border-brand-green/20 hover:shadow-premium transition-all duration-300 flex flex-col items-start"
            whileHover={{ y: -4 }}
          >
            <div className="w-10 h-10 rounded-xl bg-brand-green-light flex items-center justify-center text-brand-green-deep mb-4">
              <Icon className="w-5 h-5" />
            </div>
            <h4 className="text-sm font-black text-brand-green-deep tracking-tight mb-2">
